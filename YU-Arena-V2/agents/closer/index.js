...
async function claimOpportunity(opportunityId) {
  try {
    const res = await fetch(`${API_BASE}/opportunities/${opportunityId}/claim`, {
      method: "POST",
      headers,
    });
...
async function resolveOpportunity(opportunityId, title) {
  try {
    await fetch(`${API_BASE}/opportunities/${opportunityId}/resolve`, {
      method: "POST",
      headers,
...
function connect() {
  ...
  ws.on("open", async () => {
    ...
    await postMessage("Closer online. Waiting for Scout recommendations...");
  });
  ...
  ws.on("message", async (raw) => {
    try {
      ...
      if (event.type === "scout_decision_made") {
        ...
        if (payload.decision !== "CLAIM") return;
        ...
        await resolveOpportunity(payload.opportunity_id, payload.title || payload.opportunity_id);
        await postMessage(`Resolved: "${payload.opportunity_id}" — capacity recovered.`);
      }
    } catch (err) {
